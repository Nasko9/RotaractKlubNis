...
router.get('/', async (req, res, next) => {
  ...
  const akcije = await Akcija.find({}).sort({_id: -1}).skip((perPage * page) - perPage).limit(perPage).exec(function(err, akcije) {
      Akcija.countDocuments().exec(function(err, count) {
          if (err) {
            return next(err);
          }
